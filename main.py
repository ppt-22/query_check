...
def main(args):
    ...
    if args.macros:
        macros_path = os.path.join(
            existing_data["TAP_path"], "rules/extended/macros/macros.yaml")
    ...
    if args.option == "fetch":
        print("fetching data to rule_data.json")
        subprocess.call(['python3', get_data_file])
        exit(0)
    ...
    b = datetime(current_time.year, current_time.month, current_time.day,
                 current_time.hour, current_time.minute, current_time.second)

    last_check = existing_data['timestamp'].split(" ")
    a = datetime(int(last_check[0]), int(last_check[1]), int(last_check[2]), int(
        last_check[3]), int(last_check[4]), int(last_check[5]))
    ...
    if args.strict:
        for i in df:
            matches_s = [x for x in strict_check if re.search(
                rf"\b{x.lower()}\b", df[i].lower())]
    ...
    if args.lenient:
        if args.strict:
            for i in s_d:
                matches_l = [x for x in lenient_check if re.search(
                    rf"\b{x.lower()}\b", s_d[i]["query"].lower())]
                if i != "1.1.2784" and len(matches_l) != 0:
                    sample_data_2.append(
                        [i, s_d[i]["query"], strict_check + matches_l])
                    p_2.append(i)
        else:
            for i in df:
                matches_l = [x for x in lenient_check if re.search(
    ...
    if args.regex:
        if args.strict:
            print(args.strict)
            for i in s_d:
                matches_r = [x for x in regex_check if re.search(
                    rf"{x}", df[i].lower())]
                if i != "1.1.2784" and len(matches_r) != 0:
                    sample_data_3.append(
                        [i, s_d[i]["query"], strict_check + matches_r])
                    p_3.append(i)
        else:
            for i in df:
                matches_r = [x for x in regex_check if re.search(
...
if __name__ == "__main__":
    ...
    parser.add_argument('-s', '--strict', nargs="*", required=False,
                        help="Strict search. The keywords after the flag will be checked strictly. This flag is mandatory")
    parser.add_argument('-l', '--lenient', nargs="*", required=False,
                        help="Lenient search. The keywords after the flag will be not be checked strictly")
    parser.add_argument('-re', '--regex',  nargs="*",
                        required=False, help="Search using regular expression")
    parser.add_argument('-a', '--all',  action="store_true", required=False,
                        help="Exclude results with only strict check keywords")
    parser.add_argument('-m', '--macros',  nargs="*",
                        required=False, help="For macros")
    parser.add_argument(
        "option", help="Represents the update function", nargs="?")
